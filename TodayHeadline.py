...
def main(offset):
    ...
    if json:
        for item in get_images(json):
            ...
            find_title = cursor.execute(sql_find)
            if find_title:
                pass
            else:
                # 插入数据到头条表
                effect_row = cursor.execute("insert into toutiao(title,media_name,datetime,image01,image02,image03,tag_id,type)values(%s,%s,%s,%s,%s,%s,%s,%s)",
                                            (item.get('title'), item.get('media_name'), item.get('datetime'), item.get('image01'), item.get('image02'), item.get('image03'), item.get('tag_id'), type))
                # 提交
                conn.commit()
                print(item)
